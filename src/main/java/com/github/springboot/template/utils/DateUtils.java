...
public class DateUtils {
    ...
    private final static String DEFAULT_PATTERN = "yyyyMMdd";
    ...
    public static String dateToStr(String pattern) {
        return new SimpleDateFormat(ObjectUtils.isEmpty(pattern) ? DEFAULT_PATTERN : pattern).format(now());
    }

    /**
     * 日期转换字符串
     *
     * @return 默认当前日期, yyyyMMdd 格式转换 字符串
     */
    public static String dateToStr() {
        return new SimpleDateFormat(DEFAULT_PATTERN).format(now());
    }
    ...
    public static final String DEFAULT_UNSIGNED_DATETIME = "yyyyMMddHHmm";
     ...
     * 无格式时间 例如: 094400
     */
    public static final String TIME_NOT_PATTERN = "HHmmss";
     ...
     *
     * @param date           日期 字符串格式
     * @param currentPattern 当前日期格式 #date 格式
     * @param converPattern  将要转化的日期格式
     * @return 转化后的日期 格式为 #converPattern
     */
    public static String conver(String date, String currentPattern, String converPattern) {
        Date parse;
        try {
            parse = new SimpleDateFormat(currentPattern).parse(date);
        } catch (ParseException e) {
            return "";
        }
        return new SimpleDateFormat(converPattern).format(parse);
    }

    /**
     * 获取转化格式 yyyy-MM-dd 后的日期字符串
     *
     * @param date 日期 字符串格式为 yyyyMMdd
     * @return 转化后的日期 格式为 yyyy-MM-dd
     */
    public static String conver(String date) {
        return conver(date, UNSIGNED_DATE_PATTERN, DATE_PATTERN);
    }
